...
public class LOD3MultiSolidFactory extends GeometryFactory {

    public LOD3MultiSolidFactory(CityModelView target) {
        super(target);
    }

    public LOD3MultiSolidView createLOD3MultiSolid(PlantCover gmlObject) {
        if (gmlObject.getLod3MultiSolid() == null) {
            return null;
        }

        var multiSolid = gmlObject.getLod3MultiSolid();
        return multiSolid != null ? createLOD3MultiSolid(multiSolid.getObject()) : null;
    }

    public LOD3MultiSolidView createLOD3MultiSolid(MultiSolid multiSolid) {
        var multiSolidView = new LOD3MultiSolidView(multiSolid, vertexBuffer, texCoordBuffer);
