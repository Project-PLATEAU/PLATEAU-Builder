...
/**
 * CityFurniture（地物）に対する属性の追加や削除、表示などを行うためのクラス
 */
public class CityFurnitureSchemaManager implements AttributeSchemaManager {
        private static final JsonObject ATTRIBUTE_CITYFURNITURE_CONFIG; // Road用のスキーマ
        private static final ModelType modelType = ModelType.CITY_FURNITURE;

        static {
                ATTRIBUTE_CITYFURNITURE_CONFIG = loadSchema("/CityFurnitureAttributeSchema.json");
        }
        ...
        public void addAttributeToTreeView(IFeatureView selectedFeature,
                ...
                CityFurniture selectedModel = (CityFurniture) selectedFeature.getGML();

                if (selectedModel.isSetDescription()) {
                        var attributeItem = new AttributeItem(
                                        new AttributeHandler<CityFurniture>(selectedModel,
                ...
                if (selectedModel.isSetGenericAttribute()) {
                        List<AbstractGenericAttribute> genericAttributes = ((CityFurniture) selectedModel)
                                        .getGenericAttribute();
        ...
        public String getAttributeName(String attributeKey) {
                return ATTRIBUTE_CITYFURNITURE_CONFIG.getJsonObject(attributeKey).getString("name");
        }

        public String getAttributeType(String attributeKey) {
                return ATTRIBUTE_CITYFURNITURE_CONFIG.getJsonObject(attributeKey).getString("type");
        }

        public String getAttributeMin(String attributeKey) {
                return ATTRIBUTE_CITYFURNITURE_CONFIG.getJsonObject(attributeKey).getString("min");
        }

        public String getAttributeMax(String attributeKey) {
                return ATTRIBUTE_CITYFURNITURE_CONFIG.getJsonObject(attributeKey).getString("max");
        }

        public String getAttributeUom(String attributeKey) {
                JsonObject attributeObject = ATTRIBUTE_CITYFURNITURE_CONFIG.getJsonObject(attributeKey);
        ...
        private JsonObject getChildAttribute(String parentAttributeKey, String childKey) {
                JsonObject attributeObject = ATTRIBUTE_CITYFURNITURE_CONFIG.getJsonObject(parentAttributeKey);
        ...
        public ArrayList<String> getParentNames() {
                ...
                for (String key : ATTRIBUTE_CITYFURNITURE_CONFIG.keySet()) {
                        JsonObject parentObject = ATTRIBUTE_CITYFURNITURE_CONFIG.getJsonObject(key);
        ...
        private static JsonObject loadSchema(String path) {
                try (InputStream fis = CityFurnitureSchemaManager.class.getResourceAsStream(path);
        ...
        public List<ADEComponent> initializeAttributes(IFeatureView selectedFeature) {
                ...
                selectedModel.unsetGenericApplicationPropertyOfCityFurniture();
                return getADEComponents(selectedFeature);
        }

        public List<ADEComponent> getADEComponents(IFeatureView selectedFeature) {
                CityFurniture selectedModel = (CityFurniture) selectedFeature.getGML();
                return selectedModel.getGenericApplicationPropertyOfCityFurniture();
        }
}
