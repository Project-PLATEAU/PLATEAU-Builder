...
public class LOD2GeometryFactory extends GeometryFactory {
    public LOD2GeometryFactory(CityModelView target) {
        super(target);
    }

    public LOD2GeometryView createLOD2Geometry(SolitaryVegetationObject gmlObject) {
        var geometry = gmlObject.getLod2Geometry();
        return geometry != null ? createLOD2Geometry(geometry.getObject()) : null;
    }

    public LOD2GeometryView createLOD2Geometry(CityFurniture gmlObject) {
        var geometry = gmlObject.getLod2Geometry();
        return geometry != null ? createLOD2Geometry(geometry.getObject()) : null;
    }

    public LOD2GeometryView createLOD2Geometry(BuildingInstallation gmlObject) {
        var geometry = gmlObject.getLod2Geometry();
        return geometry != null ? createLOD2Geometry(geometry.getObject()) : null;
    }

    public LOD2GeometryView createLOD2Geometry(AbstractGeometry geometry) {
        var geometryView = new LOD2GeometryView(geometry, vertexBuffer, texCoordBuffer);
