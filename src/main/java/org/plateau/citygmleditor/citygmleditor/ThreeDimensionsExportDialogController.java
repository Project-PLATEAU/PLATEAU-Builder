...
public class ThreeDimensionsExportDialogController implements Initializable {
    ...
    public void onSelectFile(ActionEvent actionEvent) {
        var type = exportType == ExportTypeEnum.OBJ ? "OBJ" : "gLTF";
        var extensions = exportType == ExportTypeEnum.OBJ ? new String[] {"*.obj"} : new String[] {"*.gltf", "*.glb"};
        FileChooser chooser = new FileChooser();
        chooser.getExtensionFilters().addAll(
                new FileChooser.ExtensionFilter(type, extensions)
        );
        chooser.setTitle(String.format("Export %s", type));
        File newFile = chooser.showSaveDialog(this.root.getOwner());
        if (newFile == null)
            return;

        textFieldFile.setText(newFile.getAbsolutePath());
    }
    ...
    public void onSubmit(ActionEvent actionEvent) {
        ...
        this.convertOption = new ConvertOption(
            new Vec3d(
     ...
     * @param buildingView
     */
    public static ThreeDimensionsExportDialogController create(BuildingView buildingView, ExportTypeEnum exportType) {
        try {
            ...
            controller.setExportType(exportType);
            stage.showAndWait();
