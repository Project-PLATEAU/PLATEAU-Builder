...
## 1. 本書について
本書では、PLATEAU Builder（以下「本ツール」という）の画面・使い方・扱うデータについて記載しています。
...
### ⑤フィルターボタン
地物型や主題属性でフィルターを行います。ボタン押下後に「2-5.フィルター画面」へ移動します。

設定したフィルターはクリアボタン押下でクリアできます。
### ⑥3Dビューエリア
インポートしたデータを3Dで描画します。
...
## 2-5. フィルタ－画面
地物型や主題属性で検索し、3Dビューエリアと地物詳細エリアに表示する地物をフィルターできます。
表示条件を設定し、検索するボタンを押下後にフィルターが実行されます。

フィルタ－画面では以下の項目が設定できます。

|項目|説明|必須|
|---|---|---|
|地物型|表示する地物型を指定します。|○|
|主題属性|主題属性を指定します。|×|
|条件|主題属性を検索する条件を指定します。|主題属性を指定した場合は必須|
|値|主題属性を検索する値を指定します。|主題属性を指定した場合は必須|
...
1. 本ツール上に表示された属性情報を編集する方法
2. 属性情報を CSV ファイルに出力し、編集後に本ツールに取り込むことで地物に反映する方法

### 3-4-1. 本ツール上で直接編集する方法

トップ画面の地物一覧エリアまたは 3D ビューエリアで地物を選択すると、地物詳細エリアに該当する地物の情報が表示されます。  
※複数の地物を選択している場合は共通する属性のみが表示され、編集・追加・削除の操作は選択中全ての地物に適用されます。
...
※複数の地物を選択中の場合は、全ての地物で追加可能な属性のみを表示します。
...
### 3-4-2. CSV を用いて編集する方法

#### エクスポート

CSV タブからエクスポートを選ぶことで、エクスポート用の画面が表示されます。
![image](../resources/HowToUse/csv_export1.png)

出力先を選択し、エクスポートボタンを押すことでエクスポートが可能です。  
※デフォルトでは、読み込んでいる CityGML ファイルの直下に csv フォルダを作成し、そのフォルダ配下に csv ファイルをエクスポートする設定となっています。

![image](../resources/HowToUse/csv_export2.png)

#### インポート

CSV タブからインポートを選ぶことで、インポート用の画面が表示されます。

![image](../resources/HowToUse/csv_import1.png)

編集後の CSV ファイルを選択し、インポートボタンを押すことでインポート可能です。
問題なく取り込めた場合は、「対象の CSV の読み込みが完了しました。」と表示されます。なお、取り込みの際のバリデーションチェックで問題があった場合は、エラー文が表示されます。
![image](../resources/HowToUse/csv_import2.png)

#### CSV ファイルのフォーマット

-   gml:id をキーとした 1 地物 1 レコード
-   一行目はヘッダ行とし、地物の属性情報は二行目以降に記載
-   ヘッダ行
    -   同じ属性がある場合は、重複した項目について“\_[連番]”を属性名に付与。※連番は 1 から開始
    -   階層構造がある場合は、親属性名と子属性名を”\_”で連結
    -   uom や codeSpace は、属性名と付加情報名を”\_”で連結
-   属性情報行
    -   対象の地物がその属性を持たない場合は空白

#### 例

-   uro:buildingDetailAttribute が 2 つある場合は、 uro:buildingDetailAttribute_1,uro:buildingDetailAttribute_2 が存在
-   uro:buildingIDAttribute_uro:buildingID：uro:buildingIDAttribute を親とする uro:buildingID を表す属性カラム
-   bldg:measuredHeight_uom：measuredHeight の uom を設定するカラム
...
対象の地物からエクスポートした CSV より、削除したい属性カラムの削除を行い、再インポートすることで属性の一括削除が可能です。
