...
## 3-3. [WIP]地物に3Dオブジェクトを当てはめる
他ツールなどで作成済みの3Dオブジェクトを地物に当てはめることができます。

### 地物の3Dオブジェクトデータをエクスポートする
3Dオブジェクトの作成にあたって、本ツール内の地物をもとに作成したい場合は、本ツールから3Dオブジェクトをエクスポートします。
※本ツール内の地物をもとに作成しない場合には、こちらの実施は不要です。

トップ画面-地物一覧エリアにて右クリックを押下して、objファイルもしくはgLTFファイルをエクスポートします。

（キャプチャ）

### 3Dオブジェクトデータをインポートする
トップ画面-地物一覧エリアにて右クリックを押下して、objファイルもしくはgLTFファイルをインポートします。

（キャプチャ）

## 3-4. [WIP]地物の属性を編集する
トップ画面-地物一覧エリアもしくは3Dビューエリアにて該当地物を選択すると、地物詳細エリアに詳細情報が表示されます。

（キャプチャ）

既存項目を編集したい場合は、値の欄をダブルクリックして、値を修正します。

（キャプチャ）

新規項目を追加したい場合は、プラスボタンを押下して、追加する項目を選択します。

（キャプチャ）

テクスチャを変更したい場合は、プラスボタンを押下して、テクスチャとして利用する画像ファイルを選択します。

（キャプチャ）

## 3-5. [WIP]地物の面（屋根面・壁面など）を編集する
...
トップ画面にて、品質検査ボタン押下後に品質検査パラメータと出力フォルダを選択し、開くボタンを押下します。
...
実行ボタンを押下します。
画面上の品質検査の進捗ログを確認し、完了しましたら指定した出力フォルダにて結果ファイルを確認します。

![image](../resources/HowToUse/evaluation.png)

## 3-7. CityGMLのデータセットをエクスポートする
トップ画面にて、書き出しボタン押下後に出力フォルダを選択し、開くボタンを押下します。

![image](../resources/HowToUse/citygml_export②.png)

実行ボタンを押下します。
指定した出力フォルダにてデータセットを確認します。
